...
#ifdef  CARDV_UI_APP_SPEECH_ENABLE
...
static MI_S32 SSTAR_AudioInStart()
{
    ...
    for (unsigned int i = 0; i < sizeof(stAiVqeConfig.stEqCfg.s16EqGainDb) / sizeof(stAiVqeConfig.stEqCfg.s16EqGainDb[0]); i++)
    {
    ...
    for (unsigned int i = 0; i < sizeof(stAiVqeConfig.stAecCfg.u32AecSupIntensity) / sizeof(stAiVqeConfig.stAecCfg.u32AecSupIntensity[0]); i++)
	{
    ...
    for (MI_U32 i = 0; i < stAiSetAttr.u32ChnCnt; i++)
    {
