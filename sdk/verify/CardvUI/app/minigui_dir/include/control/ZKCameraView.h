...
public:
	 ...
	 * @brief 设置节点路径，默认动态识别
	 */
	void setDevPath(const char *dev);
	...
	/**
	 * @brief 使能video stream
	 */
	ECameraStatusCode startStream();

	/**
	 * @brief 停止video stream
	 */
	ECameraStatusCode stopStream();

	/**
	 * @brief 是否已使能video stream
	 */
	 ...
	 * @brief 显示/隐藏图层
	 */
	void renderToDisplay(bool show);
