...
void ffio_skip(AVIOContext *s, int count)
{
    while (count > 0) {
        int len = FFMIN(s->buf_end - s->buf_ptr, count);
        s->buf_ptr += len;
...
static int io_get_handle(void *opaque)
{
    AVIOInternal *internal = opaque;
    return ffurl_get_file_handle(internal->h);
}
