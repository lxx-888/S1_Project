...
static const AVOption options[] = {
    ...
    { "flush_moov", "Flush moov every keyframe, moov_size need to be set", 0, AV_OPT_TYPE_CONST, {.i64 = FF_MOV_FLAG_FLUSH_MOOV_KEYFRAME}, INT_MIN, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM, "movflags" },
    ...
    { "use_editlist", "use edit list", offsetof(MOVMuxContext, use_editlist), AV_OPT_TYPE_BOOL, {.i64 = 0}, -1, 1, AV_OPT_FLAG_ENCODING_PARAM},
...
#define SSTAR_DEBUG (0)
#define SSTAR_DEBUG_MOOV_SIZE (0)
#define SSTAR_DEBUG_TRAK_SIZE (0)
#define SSTAR_DEBUG_MDIA_SIZE (0)
#define SSTAR_DEBUG_MINF_SIZE (0)
#define SSTAR_DEBUG_GMHD_SIZE (0)
#define SSTAR_DEBUG_HDLR_SIZE (0)
#define SSTAR_DEBUG_TCMI_SIZE (0)
#define SSTAR_DEBUG_STBL_SIZE (0)
#define SSTAR_DEBUG_STSD_SIZE (0)
#define SSTAR_DEBUG_STSS_SIZE (0)
#define SSTAR_DEBUG_STSC_SIZE (0)
#define SSTAR_DEBUG_STSZ_SIZE (0)
#define SSTAR_DEBUG_STCO_SIZE (0)
#define SSTAR_DEBUG_ESDS_SIZE (0)
#define SSTAR_DEBUG_WAVE_SIZE (0)
#define SSTAR_DEBUG_HVCC_SIZE (0)
#define SSTAR_DEBUG_AVCC_SIZE (0)
#define SSTAR_DEBUG_AUDIO_TAG_SIZE (0)
#define SSTAR_DEBUG_VIDEO_TAG_SIZE (0)

#define PAGE_SIZE   (0x1000) // 4096 bytes
#define BLOCK_SIZE  (0x200)  // 512 bytes
#define CLUSTER_SIZE (0x10000) // FAT32 Max cluster size 64KB, exFAT max 256KB

#define FLUSH_MOOV_INTERVAL (90000 * 1) // 1 seconds
...
static int mov_calc_stco_tag_size(MOVTrack *track)
{
    int i;
...
static int mov_write_stco_tag(AVIOContext *pb, MOVTrack *track)
{
    int i;
    int mode64 = co64_required(track); // use 32 bit size variant if possible
#if (SSTAR_DEBUG_STCO_SIZE)
    int64_t pos = avio_tell(pb);
#endif
    avio_wb32(pb, track->stco_size); /* size */
    if (mode64)
...
#if (SSTAR_DEBUG_STCO_SIZE)
    printf("stco%u size = %lld, calc = %d\n", track->track_id, avio_tell(pb) - pos, track->stco_size);
#endif
    return track->stco_size;
}

static int mov_calc_stsz_tag_size(MOVTrack *track)
{
    int equalChunks = 1;
    int i, entries = 0, tst = -1, oldtst = -1;
    ...
    if (equalChunks && track->entry) {
        ...
        track->stsz_size += 8;
        for (i = 0; i < track->entry; i++) {
            track->stsz_size += track->cluster[i].entries * 4;
        }
    }

    return track->stsz_size;
}
...
#if (SSTAR_DEBUG_STSZ_SIZE)
    int64_t pos = avio_tell(pb);
#endif
    avio_wb32(pb, track->stsz_size); /* size */
...
#if (SSTAR_DEBUG_STSZ_SIZE)
    printf("stsz%u size = %lld, calc = %d\n", track->track_id, avio_tell(pb) - pos, track->stsz_size);
#endif
    return track->stsz_size;
}

static int mov_calc_stsc_tag_size(MOVTrack *track)
{
    int oldval = -1, i;
...
static int mov_write_stsc_tag(AVIOContext *pb, MOVTrack *track)
{
...
#if (SSTAR_DEBUG_STSC_SIZE)
    int64_t pos = avio_tell(pb);
#endif
    avio_wb32(pb, track->stsc_size); /* size */
    ...
    avio_wb32(pb, index); // entry count
    oldval = -1;
    for (i = 0; i < track->entry; i++) {
        if (oldval != track->cluster[i].samples_in_chunk && track->cluster[i].chunkNum) {
...
#if (SSTAR_DEBUG_STSC_SIZE)
    printf("stsc%u size = %lld, calc = %d\n", track->track_id, avio_tell(pb) - pos, track->stsc_size);
#endif
    return track->stsc_size;
}

static int mov_calc_stss_tag_size(MOVTrack *track, uint32_t flag)
{
    int size = 16;
    int i;
    ...
    if (flag == MOV_SYNC_SAMPLE)
        track->stss_size = size;
    else
        track->stps_size = size;
...
{
    int i, index = 0;
#if (SSTAR_DEBUG_STSS_SIZE)
    int64_t pos = avio_tell(pb);
#endif
    int size = flag == MOV_SYNC_SAMPLE ? track->stss_size : track->stps_size;
...
#if (SSTAR_DEBUG_STSS_SIZE)
    if (flag == MOV_SYNC_SAMPLE)
        printf("stts%u size = %lld, calc = %d\n", track->track_id, avio_tell(pb) - pos, track->stss_size);
    else
        printf("stps%u size = %lld, calc = %d\n", track->track_id, avio_tell(pb) - pos, track->stps_size);
#endif
    return size;
}

static int mov_calc_sdtp_tag_size(MOVTrack *track)
{
    int sdtp_size = 12 + track->entry * 1;
    return sdtp_size;
}
...
static int mov_write_sdtp_tag(AVIOContext *pb, MOVTrack *track)
{
    ...
    int size = mov_calc_sdtp_tag_size(track);
    avio_wb32(pb, size); // size
...
static int mov_calc_amr_tag_size(void)
{
    return 0x11;
}
...
static int mov_calc_ac3_tag_size(MOVTrack *track)
{
    if (track->vos_len < 7)
        return 0;
    return 11;
...
static int mov_calc_eac3_tag_size(MOVTrack *track)
{
    PutBitContext pbc;
    uint8_t *buf;
    struct eac3_info *info;
...
static int mov_calc_extradata_tag_size(MOVTrack *track)
{
...
static int mov_calc_enda_tag_size(void)
{
    return 10;
}
...
static int mov_calc_esds_tag_size(MOVTrack *track)
{
    int esds_size = 0;

    esds_size += 12;
    esds_size += get_descr_len() + 3 + get_descr_len() + 2 + 3 + 8;

    if (track->vos_len) {
        esds_size += get_descr_len() + track->vos_len;
    }

    esds_size += get_descr_len() + 1;
...
static int mov_write_esds_tag(AVIOContext *pb, MOVTrack *track) // Basic
{
...
#if (SSTAR_DEBUG_ESDS_SIZE)
    int64_t pos = avio_tell(pb);
#endif
    int size = mov_calc_esds_tag_size(track);
    ...
    avio_w8(pb, 0x02);
#if (SSTAR_DEBUG_ESDS_SIZE)
    printf("esds%u size = %lld, calc = %d\n", track->track_id, avio_tell(pb) - pos, size);
#endif
    return size;
}
...
static int mov_calc_ms_tag_size(AVFormatContext *s, MOVTrack *track)
{
    int ms_size = 0;
    AVIOContext *pb_buf;
    uint8_t *buf;
    ...
    if ((ret = ff_put_wav_header(s, pb_buf, track->par, 0)) < 0)
        return 0;

    ms_size = 8 + avio_close_dyn_buf(pb_buf, &buf);
    av_free(buf);
    return ms_size;
}
...
{
    int ret;
    int size = mov_calc_ms_tag_size(s, track);
    avio_wb32(pb, size);
...
static int mov_calc_wfex_tag_size(AVFormatContext *s, MOVTrack *track)
{
    int wfex_size = 0;
    AVIOContext *pb_buf;
    uint8_t *buf;
    ...
    if ((ret = ff_put_wav_header(s, pb_buf, track->st->codecpar, FF_PUT_WAV_HEADER_FORCE_WAVEFORMATEX)) < 0)
        return 0;

    wfex_size = 8 + avio_close_dyn_buf(pb_buf, &buf);
    av_free(buf);
    return wfex_size;
}
...
{
    int ret;
    int size = mov_calc_wfex_tag_size(s, track);
    avio_wb32(pb, size);
...
static int mov_calc_dfla_tag_size(MOVTrack *track)
{
    int dfla_size = 12;
    ...
    dfla_size += 4 + track->par->extradata_size;
    return dfla_size;
}

static int mov_write_dfla_tag(AVIOContext *pb, MOVTrack *track)
{
    int size = mov_calc_dfla_tag_size(track);
    avio_wb32(pb, size);
...
static int mov_calc_dops_tag_size(MOVTrack *track)
{
    int dops_size = 9;
    ...
    dops_size += 9 + track->par->extradata_size - 18;
    return dops_size;
}

static int mov_write_dops_tag(AVIOContext *pb, MOVTrack *track)
{
    int size = mov_calc_dops_tag_size(track);
    avio_wb32(pb, size);
...
static int mov_calc_chan_tag_size(void)
{
    return 24;
}
...
static int mov_calc_wave_tag_size(AVFormatContext *s, MOVTrack *track)
{
    int wave_size = 0;
    ...
    if (track->par->codec_id == AV_CODEC_ID_AAC) {
        ...
        wave_size += 12 + mov_calc_esds_tag_size(track);
    } else if (mov_pcm_le_gt16(track->par->codec_id)) {
        wave_size += mov_calc_enda_tag_size();
    } else if (mov_pcm_be_gt16(track->par->codec_id)) {
        wave_size += mov_calc_enda_tag_size();
    } else if (track->par->codec_id == AV_CODEC_ID_AMR_NB) {
        wave_size += mov_calc_amr_tag_size();
    } else if (track->par->codec_id == AV_CODEC_ID_AC3) {
        wave_size += mov_calc_ac3_tag_size(track);
    } else if (track->par->codec_id == AV_CODEC_ID_EAC3) {
        wave_size += mov_calc_eac3_tag_size(track);
    } else if (track->par->codec_id == AV_CODEC_ID_ALAC ||
               track->par->codec_id == AV_CODEC_ID_QDM2) {
        wave_size += mov_calc_extradata_tag_size(track);
    } else if (track->par->codec_id == AV_CODEC_ID_ADPCM_MS ||
               track->par->codec_id == AV_CODEC_ID_ADPCM_IMA_WAV) {
        wave_size += mov_calc_ms_tag_size(s, track);
    }
...
{
#if (SSTAR_DEBUG_WAVE_SIZE)
    int64_t pos = avio_tell(pb);
#endif
    int size = mov_calc_wave_tag_size(s, track);
...
static int mov_calc_dvc1_tag_size(MOVTrack *track)
{
    int dvc1_size = 0;
    uint8_t buf[7] = { 0 };
    int ret;
    ...
    dvc1_size = track->vos_len + 8 + sizeof(buf);
    return dvc1_size;
}
...
static int mov_calc_glbl_tag_size(MOVTrack *track)
{
...
static int mov_calc_audio_tag_size(AVFormatContext *s, MOVMuxContext *mov, MOVTrack *track)
{
    int version = 0;
    ...
    track->audio_tag_size = 24;

    if (version == 2) {
        track->audio_tag_size += 48;
    } else {
        track->audio_tag_size += 12;
    }

    if (version == 1) { /* SoundDescription V1 extended info */
        track->audio_tag_size += 16;
    }
    ...
    if (track->mode == MODE_MOV &&
        ...
        track->audio_tag_size += mov_calc_wave_tag_size(s, track);
    else if (track->tag == MKTAG('m','p','4','a'))
        track->audio_tag_size += mov_calc_esds_tag_size(track);
    else if (track->par->codec_id == AV_CODEC_ID_AMR_NB)
        track->audio_tag_size += mov_calc_amr_tag_size();
    else if (track->par->codec_id == AV_CODEC_ID_AC3)
        track->audio_tag_size += mov_calc_ac3_tag_size(track);
    else if (track->par->codec_id == AV_CODEC_ID_EAC3)
        track->audio_tag_size += mov_calc_eac3_tag_size(track);
    else if (track->par->codec_id == AV_CODEC_ID_ALAC)
        track->audio_tag_size += mov_calc_extradata_tag_size(track);
    else if (track->par->codec_id == AV_CODEC_ID_WMAPRO)
        track->audio_tag_size += mov_calc_wfex_tag_size(s, track);
    else if (track->par->codec_id == AV_CODEC_ID_FLAC)
        track->audio_tag_size += mov_calc_dfla_tag_size(track);
    else if (track->par->codec_id == AV_CODEC_ID_OPUS)
        track->audio_tag_size += mov_calc_dops_tag_size(track);
    else if (track->vos_len > 0)
        track->audio_tag_size += mov_calc_glbl_tag_size(track);

    if (track->mode == MODE_MOV && track->par->codec_type == AVMEDIA_TYPE_AUDIO)
        track->audio_tag_size += mov_calc_chan_tag_size();

    // if (mov->encryption_scheme != MOV_ENC_NONE) {
    //     ff_mov_cenc_write_sinf_tag(track, pb, mov->encryption_kid); jeff
    // }

    return track->audio_tag_size;
}

static int mov_write_audio_tag(AVFormatContext *s, AVIOContext *pb, MOVMuxContext *mov, MOVTrack *track)
{
#if (SSTAR_DEBUG_AUDIO_TAG_SIZE)
    int64_t pos = avio_tell(pb);
#endif
    int version = 0;
    ...
    avio_wb32(pb, track->audio_tag_size); /* size */
    if (mov->encryption_scheme != MOV_ENC_NONE) {
...
#if (SSTAR_DEBUG_AUDIO_TAG_SIZE)
    printf("audio%u size = %lld, calc = %d\n", track->track_id, avio_tell(pb) - pos, track->audio_tag_size);
#endif
    return track->audio_tag_size;
}

static int mov_calc_d263_tag_size(void)
{
    return 0xf;
}
...
static int mov_calc_av1c_tag_size(MOVTrack *track)
{
    AVIOContext *pb_buf;
    uint8_t *buf;
    int ret;
    ...
    ff_isom_write_av1c(pb_buf, track->vos_data, track->vos_len);

    track->av1c_size = 8 + avio_close_dyn_buf(pb_buf, &buf);
    av_free(buf);
    return track->av1c_size;
}

static int mov_write_av1c_tag(AVIOContext *pb, MOVTrack *track)
{
    avio_wb32(pb, track->av1c_size);
    ffio_wfourcc(pb, "av1C");
    ff_isom_write_av1c(pb, track->vos_data, track->vos_len);
    return track->av1c_size;
}

static int mov_calc_avcc_tag_size(MOVTrack *track)
{
    AVIOContext *pb_buf;
    uint8_t *buf;
    int ret;
    ...
    ff_isom_write_avcc(pb_buf, track->vos_data, track->vos_len);

    track->avcc_size = 8 + avio_close_dyn_buf(pb_buf, &buf);
    av_free(buf);
    return track->avcc_size;
}

static int mov_write_avcc_tag(AVIOContext *pb, MOVTrack *track)
{
#if (SSTAR_DEBUG_AVCC_SIZE)
    int64_t pos = avio_tell(pb);
#endif

    avio_wb32(pb, track->avcc_size);
    ffio_wfourcc(pb, "avcC");
    ff_isom_write_avcc(pb, track->vos_data, track->vos_len);
#if (SSTAR_DEBUG_AVCC_SIZE)
    printf("avcc%u size = %lld, calc = %d\n", track->track_id, avio_tell(pb) - pos, track->avcc_size);
#endif
    return track->avcc_size;
}

static int mov_calc_vpcc_tag_size(AVFormatContext *s, MOVTrack *track)
{
    AVIOContext *pb_buf;
    uint8_t *buf;
    int ret;
    ...
    ff_isom_write_vpcc(s, pb_buf, track->par);

    track->vpcc_size = 12 + avio_close_dyn_buf(pb_buf, &buf);
    av_free(buf);
    return track->vpcc_size;
}

static int mov_write_vpcc_tag(AVFormatContext *s, AVIOContext *pb, MOVTrack *track)
{
    avio_wb32(pb, track->vpcc_size);
...
static int mov_calc_hvcc_tag_size(MOVTrack *track)
{
    AVIOContext *pb_buf;
    uint8_t *buf;
    int ret;
    ...
    if (track->tag == MKTAG('h','v','c','1'))
        ff_isom_write_hvcc(pb_buf, track->vos_data, track->vos_len, 1);
    else
        ff_isom_write_hvcc(pb_buf, track->vos_data, track->vos_len, 0);

    track->hvcc_size = 8 + avio_close_dyn_buf(pb_buf, &buf);
    av_free(buf);
    return track->hvcc_size;
}

static int mov_write_hvcc_tag(AVIOContext *pb, MOVTrack *track)
{
#if (SSTAR_DEBUG_HVCC_SIZE)
    int64_t pos = avio_tell(pb);
#endif

    avio_wb32(pb, track->hvcc_size);
...
#if (SSTAR_DEBUG_HVCC_SIZE)
    printf("hvcc%u size = %lld, calc = %d\n", track->track_id, avio_tell(pb) - pos, track->hvcc_size);
#endif
    return track->hvcc_size;
}

static int mov_calc_avid_tag_size(MOVTrack *track)
{
    int avid_size = 0;
...
static int mov_calc_dpxe_tag_size(void)
{
    return 12;
}
...
static int mov_calc_uuid_tag_ipod_size(void)
{
    return 28;
}
...
static int mov_calc_fiel_tag_size(int field_order)
{
...
static int mov_calc_subtitle_tag_size(MOVTrack *track)
{
    int subtitle_size = 16;

    if (track->par->codec_id == AV_CODEC_ID_DVD_SUBTITLE)
        subtitle_size += mov_calc_esds_tag_size(track);
    else if (track->par->extradata_size)
        subtitle_size += track->par->extradata_size;
...
{
    int size = mov_calc_subtitle_tag_size(track);
    avio_wb32(pb, size);    /* size */
    avio_wl32(pb, track->tag); // store it byteswapped
    avio_wb32(pb, 0);    /* Reserved */
    avio_wb16(pb, 0);    /* Reserved */
...
static int mov_calc_st3d_tag_size(void)
{
    return 13;
}
...
static int mov_calc_sv3d_tag_size(AVFormatContext *s, AVSphericalMapping *spherical_mapping)
{
    int sv3d_size = 0;
    const char* metadata_source = s->flags & AVFMT_FLAG_BITEXACT ? "Lavf" : LIBAVFORMAT_IDENT;

    sv3d_size = 8 + 12 + (1 + strlen(metadata_source)) + 24;
...
{
    const char* metadata_source = s->flags & AVFMT_FLAG_BITEXACT ? "Lavf" : LIBAVFORMAT_IDENT;
    int sv3d_size = mov_calc_sv3d_tag_size(s, spherical_mapping);
    int svhd_size = 12 + (1 + strlen(metadata_source));
    int proj_size = sv3d_size - svhd_size;
...
static int mov_calc_clap_tag_size(void)
{
    return 40;
}
...
static int mov_calc_pasp_tag_size(void)
{
    return 16;
}
...
static int mov_calc_gama_tag_size(MOVTrack *track, double gamma)
{
    if (gamma <= 0.0)
    {
...
static int mov_calc_colr_tag_size(MOVTrack *track)
{
...
static int mov_calc_video_tag_size(MOVMuxContext *mov, MOVTrack *track)
{
    int avid = 0;
    ...
    track->video_tag_size = 16 + 4 + 12 + 18 + 32 + 2;

    if (track->mode == MODE_MOV && track->par->format == AV_PIX_FMT_PAL8) {
        int pal_size = 1 << track->par->bits_per_coded_sample;
        int i;
        track->video_tag_size += 10;
        for (i = 0; i < pal_size; i++) {
            track->video_tag_size += 8;
        }
    } else
        track->video_tag_size += 2; /* Reserved */

    if (track->tag == MKTAG('m','p','4','v'))
        track->video_tag_size += mov_calc_esds_tag_size(track);
    else if (track->par->codec_id == AV_CODEC_ID_H263)
        track->video_tag_size += mov_calc_d263_tag_size();
    else if (track->par->codec_id == AV_CODEC_ID_AVUI ||
            track->par->codec_id == AV_CODEC_ID_SVQ3) {
        track->video_tag_size += mov_calc_extradata_tag_size(track) + 4;
    } else if (track->par->codec_id == AV_CODEC_ID_DNXHD) {
        track->video_tag_size += mov_calc_avid_tag_size(track);
        avid = 1;
    } else if (track->par->codec_id == AV_CODEC_ID_HEVC)
        track->video_tag_size += mov_calc_hvcc_tag_size(track);
    else if (track->par->codec_id == AV_CODEC_ID_H264 && !TAG_IS_AVCI(track->tag)) {
        track->video_tag_size += mov_calc_avcc_tag_size(track);
        if (track->mode == MODE_IPOD)
            track->video_tag_size += mov_calc_uuid_tag_ipod_size();
    } else if (track->par->codec_id == AV_CODEC_ID_VP9) {
        track->video_tag_size += mov_calc_vpcc_tag_size(mov->fc, track);
    } else if (track->par->codec_id == AV_CODEC_ID_AV1) {
        track->video_tag_size += mov_calc_av1c_tag_size(track);
    } else if (track->par->codec_id == AV_CODEC_ID_VC1 && track->vos_len > 0)
        track->video_tag_size += mov_calc_dvc1_tag_size(track);
        ...
        if (track->par->codec_tag == MKTAG('R','1','0','k'))
            track->video_tag_size += mov_calc_dpxe_tag_size();
    } else if (track->vos_len > 0)
        track->video_tag_size += mov_calc_glbl_tag_size(track);
    ...
    FF_ENABLE_DEPRECATION_WARNINGS
        ...
        if (field_order != AV_FIELD_UNKNOWN)
            track->video_tag_size += mov_calc_fiel_tag_size(field_order);
    }

    if (mov->flags & FF_MOV_FLAG_WRITE_GAMA) {
        if (track->mode == MODE_MOV)
            track->video_tag_size += mov_calc_gama_tag_size(track, mov->gamma);
        else
            av_log(mov->fc, AV_LOG_WARNING, "Not writing 'gama' atom. Format is not MOV.\n");
    }
    if (mov->flags & FF_MOV_FLAG_WRITE_COLR) {
        if (track->mode == MODE_MOV || track->mode == MODE_MP4)
            track->video_tag_size += mov_calc_colr_tag_size(track);
        else
    ...
    if (track->mode == MODE_MP4 && mov->fc->strict_std_compliance <= FF_COMPLIANCE_UNOFFICIAL) {
        ...
        if (stereo_3d)
            track->video_tag_size += mov_calc_st3d_tag_size();
        if (spherical_mapping)
            track->video_tag_size += mov_calc_sv3d_tag_size(mov->fc, spherical_mapping);
    }

    if (track->par->sample_aspect_ratio.den && track->par->sample_aspect_ratio.num) {
        track->video_tag_size += mov_calc_pasp_tag_size();
    }

    if (uncompressed_ycbcr){
        track->video_tag_size += mov_calc_clap_tag_size();
    }
    ...
    if (avid)
        track->video_tag_size += 4;

    return track->video_tag_size;
}

static int mov_write_video_tag(AVIOContext *pb, MOVMuxContext *mov, MOVTrack *track)
{
#if (SSTAR_DEBUG_VIDEO_TAG_SIZE)
    int64_t pos = avio_tell(pb);
#endif
    ...
    avio_wb32(pb, track->video_tag_size); /* size */
    if (mov->encryption_scheme != MOV_ENC_NONE) {
...
#if (SSTAR_DEBUG_VIDEO_TAG_SIZE)
    printf("video%u size = %lld, calc = %d\n", track->track_id, avio_tell(pb) - pos, track->video_tag_size);
#endif
    return track->video_tag_size;
}

static int mov_calc_rtp_tag_size(void)
{
    return 36;
}
...
static int mov_calc_source_reference_tag_size(const char *reel_name)
{
...
static int mov_calc_tmcd_tag_size(MOVTrack *track)
{
    int tmcd_size = 34;
    ...
    if (t && utf8len(t->value) && track->mode != MODE_MP4)
        tmcd_size += mov_calc_source_reference_tag_size(t->value);
    else
        tmcd_size += 2; /* zero size */
...
{
    int size = mov_calc_tmcd_tag_size(track);
#if 1
    int frame_duration;
    int nb_frames;
    AVDictionaryEntry *t = NULL;
...
static int mov_calc_ssmd_tag_size(void)
{
    return 20;
}

static int mov_write_ssmd_tag(AVIOContext *pb, const MOVTrack *track)
{
    avio_wb32(pb, 20); /* size */
    ffio_wfourcc(pb, "ssmd");
    avio_wb32(pb, 0); /* Reserved */
    avio_wb16(pb, 0); /* Reserved */
    avio_wb16(pb, 1); /* Data-reference index */
    avio_wb32(pb, 0); /* Reserved */
...
static int mov_calc_stsd_tag_size(AVFormatContext *s, MOVMuxContext *mov, MOVTrack *track)
{
    track->stsd_size = 16;
    if (track->par->codec_type == AVMEDIA_TYPE_VIDEO)
        track->stsd_size += mov_calc_video_tag_size(mov, track);
    else if (track->par->codec_type == AVMEDIA_TYPE_AUDIO)
        track->stsd_size += mov_calc_audio_tag_size(s, mov, track);
    else if (track->par->codec_type == AVMEDIA_TYPE_SUBTITLE)
        track->stsd_size += mov_calc_subtitle_tag_size(track);
    else if (track->par->codec_tag == MKTAG('r','t','p',' '))
        track->stsd_size += mov_calc_rtp_tag_size();
    else if (track->par->codec_tag == MKTAG('t','m','c','d'))
        track->stsd_size += mov_calc_tmcd_tag_size(track);
    else if (track->par->codec_tag == MKTAG('s','s','m','d'))
        track->stsd_size += mov_calc_ssmd_tag_size();

    return track->stsd_size;
}

static int mov_write_stsd_tag(AVFormatContext *s, AVIOContext *pb, MOVMuxContext *mov, MOVTrack *track)
{
#if (SSTAR_DEBUG_STSD_SIZE)
    int64_t pos = avio_tell(pb);
#endif
    avio_wb32(pb, track->stsd_size); /* size */
    ...
    if (track->par->codec_type == AVMEDIA_TYPE_VIDEO)
        ...
        mov_write_ssmd_tag(pb, track);

#if (SSTAR_DEBUG_STSD_SIZE)
    printf("stsd%u size = %lld, calc = %d\n", track->track_id, avio_tell(pb) - pos, track->stsd_size);
#endif
    return track->stsd_size;
}

static int mov_calc_ctts_tag_size(MOVTrack *track)
{
...
static int mov_calc_stts_tag_size(MOVTrack *track)
{
...
static int mov_calc_dref_tag_size(void)
{
    return 28;
}
...
static int mov_preroll_calc_stbl_atoms_size(MOVTrack *track)
{
    struct sgpd_entry {
        int count;
    ...
    return 24 + (group * 2) + 20 + (entries * 8);
}
...
static int mov_calc_stbl_tag_size(AVFormatContext *s, MOVMuxContext *mov, MOVTrack *track)
{
    track->stbl_size = 8;
    track->stbl_size += mov_calc_stsd_tag_size(s, mov, track);
    track->stbl_size += mov_calc_stts_tag_size(track);

    if ((track->par->codec_type == AVMEDIA_TYPE_VIDEO ||
         track->par->codec_tag == MKTAG('r','t','p',' ')) &&
        track->has_keyframes && track->has_keyframes < track->entry)
        track->stbl_size += mov_calc_stss_tag_size(track, MOV_SYNC_SAMPLE);
    if (track->par->codec_type == AVMEDIA_TYPE_VIDEO && track->has_disposable)
        track->stbl_size += mov_calc_sdtp_tag_size(track);
    if (track->mode == MODE_MOV && track->flags & MOV_TRACK_STPS)
        track->stbl_size += mov_calc_stss_tag_size(track, MOV_PARTIAL_SYNC_SAMPLE);
    if (track->par->codec_type == AVMEDIA_TYPE_VIDEO &&
        track->flags & MOV_TRACK_CTTS && track->entry) {
        track->stbl_size += mov_calc_ctts_tag_size(track);
    }
    track->stbl_size += mov_calc_stsc_tag_size(track);
    track->stbl_size += mov_calc_stsz_tag_size(track);
    track->stbl_size += mov_calc_stco_tag_size(track);
    // if (track->cenc.aes_ctr) {
    //     ff_mov_cenc_write_stbl_atoms(&track->cenc, pb);
    // }
    if (track->par->codec_id == AV_CODEC_ID_OPUS || track->par->codec_id == AV_CODEC_ID_AAC) {
        track->stbl_size += mov_preroll_calc_stbl_atoms_size(track);
    }

    return track->stbl_size;
}

static int mov_write_stbl_tag(AVFormatContext *s, AVIOContext *pb, MOVMuxContext *mov, MOVTrack *track)
{
#if (SSTAR_DEBUG_STBL_SIZE)
    int64_t pos = avio_tell(pb);
#endif
    int ret;

    avio_wb32(pb, track->stbl_size); /* size */
...
#if (SSTAR_DEBUG_STBL_SIZE)
    printf("stbl%u size = %lld, calc = %d\n", track->track_id, avio_tell(pb) - pos, track->stbl_size);
#endif
    return track->stbl_size;
}

static int mov_calc_dinf_tag_size(void)
{
    int dinf_size = 8 + mov_calc_dref_tag_size();
    return dinf_size;
}

static int mov_write_dinf_tag(AVIOContext *pb)
{
    int size = mov_calc_dinf_tag_size();
    avio_wb32(pb, size); /* size */
...
static int mov_calc_nmhd_tag_size(void)
{
    return 12;
}
...
static int mov_calc_tcmi_size(MOVTrack *track)
{
    const char *font = "Lucida Grande";
    track->tcmi_size = 32 + 1 + strlen(font);
    return track->tcmi_size;
}

static int mov_write_tcmi_tag(AVIOContext *pb, MOVTrack *track)
{
#if (SSTAR_DEBUG_TCMI_SIZE)
    int64_t pos = avio_tell(pb);
#endif
    const char *font = "Lucida Grande";
    avio_wb32(pb, track->tcmi_size);    /* size */
...
#if (SSTAR_DEBUG_TCMI_SIZE)
    printf("tcmi%u size = %lld, calc = %d\n", track->track_id, avio_tell(pb) - pos, track->tcmi_size);
#endif
    return track->tcmi_size;
}

static int mov_calc_gmhd_tag_size(MOVTrack *track)
{
    track->gmhd_size = 32;
    ...
    if (track->par->codec_tag == MKTAG('t','m','c','d')) {
        track->gmhd_size += 8 + mov_calc_tcmi_size(track);
    } else if (track->par->codec_tag == MKTAG('s','s','m','d')) {
        track->gmhd_size += 12;
    }

    return track->gmhd_size;
}

static int mov_write_gmhd_tag(AVIOContext *pb, MOVTrack *track)
{
#if (SSTAR_DEBUG_GMHD_SIZE)
    int64_t pos = avio_tell(pb);
#endif
    avio_wb32(pb, track->gmhd_size);      /* size */
    ...
    if (track->par->codec_tag == MKTAG('t','m','c','d')) {
        avio_wb32(pb, track->tcmi_size + 8); /* size */
...
#if (SSTAR_DEBUG_GMHD_SIZE)
    printf("gmhd%u size = %lld, calc = %d\n", track->track_id, avio_tell(pb) - pos, track->gmhd_size);
#endif
    return track->gmhd_size;
}

static int mov_calc_smhd_tag_size(void)
{
    return 16;
}
...
static int mov_calc_vmhd_tag_size(void)
{
    return 0x14;
}
...
static int mov_calc_hdlr_tag_size(AVFormatContext *s, MOVTrack *track)
{
    MOVMuxContext *mov = s->priv_data;
    ...
    if (!track || track->mode == MODE_MOV)
        hdlr_size += 1; /* pascal string */
    hdlr_size += strlen(descr); /* handler description */
    if (track && track->mode != MODE_MOV)
        hdlr_size += 1; /* c string */

    if (track)
        track->hdlr_size = hdlr_size;
...
static int mov_write_hdlr_tag(AVFormatContext *s, AVIOContext *pb, MOVTrack *track)
{
    ...
    if (track) {
        ...
        size = track->hdlr_size;
    } else {
        size = mov_calc_hdlr_tag_size(s, track);
    }
...
#if (SSTAR_DEBUG_HDLR_SIZE)
    if (track)
        printf("hdlr%u size = %lld, calc = %d\n", track->track_id, avio_tell(pb) - pos, track->hdlr_size);
    else
        printf("hdlr size = %lld, calc = %d\n", avio_tell(pb) - pos, size);
#endif
    return size;
}

static int mov_calc_hmhd_tag_size(void)
{
    return 28;
}
...
static int mov_calc_minf_tag_size(AVFormatContext *s, MOVMuxContext *mov, MOVTrack *track)
{
    track->minf_size = 8;
    if (track->par->codec_type == AVMEDIA_TYPE_VIDEO)
        track->minf_size += mov_calc_vmhd_tag_size();
    else if (track->par->codec_type == AVMEDIA_TYPE_AUDIO)
        track->minf_size += mov_calc_smhd_tag_size();
    else if (track->par->codec_type == AVMEDIA_TYPE_SUBTITLE) {
        if (track->tag == MKTAG('t','e','x','t') || is_clcp_track(track)) {
            track->minf_size += mov_calc_gmhd_tag_size(track);
        } else {
            track->minf_size += mov_calc_nmhd_tag_size();
        }
    } else if (track->tag == MKTAG('r','t','p',' ')) {
        track->minf_size += mov_calc_hmhd_tag_size();
    } else if (track->tag == MKTAG('t','m','c','d')) {
        if (track->mode != MODE_MOV)
            track->minf_size += mov_calc_nmhd_tag_size();
        else
            track->minf_size += mov_calc_gmhd_tag_size(track);
    } else if (track->tag == MKTAG('s','s','m','d')) {
        track->minf_size += mov_calc_gmhd_tag_size(track);
    }

    if (track->mode == MODE_MOV) /* ISO 14496-12 8.4.3.1 specifies hdlr only within mdia or meta boxes */
        track->minf_size += mov_calc_hdlr_tag_size(s, NULL);

    track->minf_size += mov_calc_dinf_tag_size();
    track->minf_size += mov_calc_stbl_tag_size(s, mov, track);

    return track->minf_size;
}

static int mov_write_minf_tag(AVFormatContext *s, AVIOContext *pb, MOVMuxContext *mov, MOVTrack *track)
{
#if (SSTAR_DEBUG_MINF_SIZE)
    int64_t pos = avio_tell(pb);
#endif
    int ret;

    avio_wb32(pb, track->minf_size); /* size */
...
#if (SSTAR_DEBUG_MINF_SIZE)
    printf("minf%u size = %lld, calc = %d\n", track->track_id, avio_tell(pb) - pos, track->minf_size);
#endif
    return track->minf_size;
}

static int mov_calc_mdhd_tag_size(MOVTrack *track)
{
...
static int mov_calc_mdia_tag_size(AVFormatContext *s,
                              MOVMuxContext *mov, MOVTrack *track)
{
    track->mdia_size = 8;
    track->mdia_size += mov_calc_mdhd_tag_size(track);
    track->mdia_size += mov_calc_hdlr_tag_size(s, track);
    track->mdia_size += mov_calc_minf_tag_size(s, mov, track);
    return track->mdia_size;
}

static int mov_write_mdia_tag(AVFormatContext *s, AVIOContext *pb,
                              MOVMuxContext *mov, MOVTrack *track)
{
#if (SSTAR_DEBUG_MDIA_SIZE)
    int64_t pos = avio_tell(pb);
#endif
    int ret;

    avio_wb32(pb, track->mdia_size); /* size */
...
#if (SSTAR_DEBUG_MDIA_SIZE)
    printf("mida%u size = %lld, calc = %d\n", track->track_id, avio_tell(pb) - pos, track->mdia_size);
#endif
    return track->mdia_size;
}
...
static int mov_calc_tkhd_tag_size(MOVMuxContext *mov,
...
static int mov_calc_tapt_tag_size(void)
{
    return 68;
}
...
static int mov_calc_tref_tag_size(void)
{
    return 20;
}
...
static int mov_calc_uuid_tag_psp_size(void)
{
    return 0x34;
}
...
static int mov_calc_udta_sdp_size(MOVTrack *track)
{
...
static int mov_calc_track_udta_tag_size(MOVMuxContext *mov, AVStream *st)
{
    AVIOContext *pb_buf;
    int ret, size;
    uint8_t *buf;
...
static int mov_calc_trak_tag_size(AVFormatContext *s, MOVMuxContext *mov,
                              MOVTrack *track, AVStream *st)
{
    track->trak_size = 0;
    track->trak_size += mov_calc_tkhd_tag_size(mov, track, st);

    if (track->tref_tag)
        track->trak_size += mov_calc_tref_tag_size();

    track->trak_size += mov_calc_mdia_tag_size(s, mov, track);
    if (track->mode == MODE_PSP)
        track->trak_size += mov_calc_uuid_tag_psp_size(); // PSP Movies require this uuid box
    if (track->tag == MKTAG('r','t','p',' '))
        track->trak_size += mov_calc_udta_sdp_size(track);
    if (track->mode == MODE_MOV) {
        if (track->par->codec_type == AVMEDIA_TYPE_VIDEO) {
            double sample_aspect_ratio = av_q2d(st->sample_aspect_ratio);
            if (st->sample_aspect_ratio.num && 1.0 != sample_aspect_ratio) {
                track->trak_size += mov_calc_tapt_tag_size();
            }
        }
    ...
    track->trak_size += mov_calc_track_udta_tag_size(mov, st);
    return track->trak_size;
}

static int mov_write_trak_tag(AVFormatContext *s, AVIOContext *pb, MOVMuxContext *mov,
                              MOVTrack *track, AVStream *st)
{
#if (SSTAR_DEBUG_TRAK_SIZE)
    int64_t pos = avio_tell(pb);
#endif
    ...
    avio_wb32(pb, track->trak_size); /* size */
...
#if (SSTAR_DEBUG_TRAK_SIZE)
    printf("trak%u size = %lld, calc = %d\n", track->track_id, avio_tell(pb) - pos, track->trak_size);
#endif
    return track->trak_size;
}

static int mov_calc_iods_tag_size(void)
{
    return 12 + get_descr_len() + 7;
}

static int mov_write_iods_tag(AVIOContext *pb, MOVMuxContext *mov)
{
    int i, has_audio = 0, has_video = 0;
    int size = mov_calc_iods_tag_size();
...
static int mov_calc_trex_tag_size(void)
{
    return 0x20;
}
...
static int mov_calc_mvex_tag_size(MOVMuxContext *mov)
{
    return 8 + mov_calc_trex_tag_size() * mov->nb_streams;
}

static int mov_write_mvex_tag(AVIOContext *pb, MOVMuxContext *mov)
{
    int i;
    int size = 8 + mov_calc_trex_tag_size() * mov->nb_streams;
    avio_wb32(pb, size); /* size */
...
static int mov_calc_mvhd_size(MOVMuxContext *mov)
{
...
static int mov_write_loci_tag(AVFormatContext *s, AVIOContext *pb)
{
    ...
    size = 14 + strlen(place) + 1 + 1 + 12 + strlen(astronomical_body) + 1 + 1;
    avio_wb32(pb, size);         /* size */
...
static int mov_calc_udta_tag_size(MOVMuxContext *mov, AVFormatContext *s)
{
    AVIOContext *pb_buf;
    int ret, size;
    uint8_t *buf;
...
static int mov_calc_psp_udta_tag_size(const char *str)
{
...
static int mov_calc_uuidusmt_tag_size(AVFormatContext *s)
{
    ...
    if (title) {
        ...
        if (!(s->flags & AVFMT_FLAG_BITEXACT))
            size += mov_calc_psp_udta_tag_size(LIBAVCODEC_IDENT);
        size += mov_calc_psp_udta_tag_size(title->value);
        size += mov_calc_psp_udta_tag_size("2006/04/01 11:11:11");
...
static int mov_write_uuidusmt_tag(AVIOContext *pb, AVFormatContext *s)
{
    ...
    if (title) {
        size = mov_calc_uuidusmt_tag_size(s);
        avio_wb32(pb, size); /* size placeholder*/
        ffio_wfourcc(pb, "uuid");
        ffio_wfourcc(pb, "USMT");
        ...
        size2 = size - 24;
        avio_wb32(pb, size2); /* size placeholder*/
        ffio_wfourcc(pb, "MTDT");
        avio_wb16(pb, 4);
...
static void build_one_sample_chunks(MOVTrack *trk)
{
    for (int i = 0; i < trk->entry; i++) {
        MOVIentry *chunk = &trk->cluster[i];
        if (chunk->chunkNum == 0) {
            chunk->chunkNum = 1;
            trk->chunkCount++;
        }
...
static int mov_setup_track_ids(MOVMuxContext *mov, AVFormatContext *s)
{
    ...
    if (mov->track_ids_ok && !(mov->flags & FF_MOV_FLAG_FLUSH_MOOV_KEYFRAME))
        return 0;
...
static int mov_calc_moov_size(MOVMuxContext *mov, AVFormatContext *s)
{
    int moov_size = 8;

    moov_size += mov_calc_mvhd_size(mov);
    if (mov->mode != MODE_MOV && !mov->iods_skip)
        moov_size += mov_calc_iods_tag_size();

    for (int i = 0; i < mov->nb_streams; i++) {
        if (mov->tracks[i].entry > 0 || mov->flags & FF_MOV_FLAG_FRAGMENT) {
            moov_size += mov_calc_trak_tag_size(s, mov, &(mov->tracks[i]), i < s->nb_streams ? s->streams[i] : NULL);
        }
    }

    if (mov->flags & FF_MOV_FLAG_FRAGMENT)
        moov_size += mov_calc_mvex_tag_size(mov); /* QuickTime requires trak to precede this */

    if (mov->mode == MODE_PSP)
        moov_size += mov_calc_uuidusmt_tag_size(s);
    else
        moov_size += mov_calc_udta_tag_size(mov, s);
                              ...
                              AVFormatContext *s)
{
    ...
    for (i = 0; i < mov->nb_streams; i++) {
        ...
        if (mov->tracks[i].entry) {
            if (mov->flags & FF_MOV_FLAG_FLUSH_MOOV_KEYFRAME)
                build_one_sample_chunks(&mov->tracks[i]);
            else
    ...
    moov_size = mov_calc_moov_size(mov, s);
    avio_wb32(pb, moov_size); /* size */
...
#if (SSTAR_DEBUG_MOOV_SIZE)
    printf("moov size = %lld, calc = %d\n", avio_tell(pb) - pos, moov_size);
#endif
    return moov_size;
}
...
static int mov_write_tfxd_tag(AVIOContext *pb, MOVTrack *track)
{
    ...
    avio_wb32(pb, 28 + sizeof(uuid)); /* size placeholder */
    ffio_wfourcc(pb, "uuid");
    avio_write(pb, uuid, sizeof(uuid));
    avio_w8(pb, 1);
    avio_wb24(pb, 0);
...
static int mov_write_prft_tag(AVIOContext *pb, MOVMuxContext *mov, int tracks)
{
    ...
    avio_wb32(pb, 32);                           // Size place holder
...
{
    int size = 24 + track->nb_frag_info * 19;
    int i;
...
{
    int64_t pos = avio_tell(pb);
    int extended_size = FFALIGN(pos + BLOCK_SIZE, CLUSTER_SIZE) - pos - 16;
    avio_wb32(pb, extended_size); // placeholder for data length alignment
    ffio_wfourcc(pb, "free");
    #if (SSTAR_DEBUG)
    ffio_fill(pb, 0xD, extended_size - 8);
    #else
    ffio_skip(pb, extended_size - 8);
    #endif
...
static int mov_update_mdat_size(AVIOContext *pb, MOVMuxContext *mov)
{
    avio_seek(pb, mov->mdat_pos - (BLOCK_SIZE - 8), SEEK_SET);
    #if (SSTAR_DEBUG)
    ffio_fill(pb, 0xE, BLOCK_SIZE - 16);
    #else
    ffio_skip(pb, BLOCK_SIZE - 16);
    #endif
...
static int mov_write_identification(AVIOContext *pb, AVFormatContext *s)
{
    ...
    ftyp_size = mov_write_ftyp_tag(pb,s);

    if (mov->mode != MODE_PSP) {
        // placeholder for moov/mdat address align to 4KB
        int free_size = PAGE_SIZE - ftyp_size;
        avio_wb32(pb, free_size);
        ffio_wfourcc(pb, "free");
        #if (SSTAR_DEBUG)
        ffio_fill(pb, 0xA, free_size - 8);
        #else
        ffio_skip(pb, free_size - 8);
        #endif
    }
...
static int mov_write_url_to_free(AVFormatContext *s)
{
    int size = strlen(s->url);
    /* For debug : write file name into free atom */
    avio_write(s->pb, s->url, size);
    return size;
}

static int mov_auto_flush_moov(AVFormatContext *s)
{
    MOVMuxContext *mov = s->priv_data;
    AVIOContext *pb = s->pb;
    int res = 0;
    int64_t free_size, moov_size, url_size, flush_size, skip_size;
    int64_t current_pos = avio_tell(pb);

    skip_size = FFALIGN(current_pos, BLOCK_SIZE) - current_pos;
    #if (SSTAR_DEBUG)
    ffio_fill(pb, 0x9, skip_size);
    #else
    ffio_skip(pb, skip_size);
    #endif
    mov->mdat_size += skip_size;
    current_pos = avio_tell(pb);

    /* moov size is not necessary for demuxing,
     * just write once in mov_write_trailer().
     */
    /* Write size of mdat tag */
    // mov_update_mdat_size(pb, mov);

    avio_seek(pb, mov->reserved_header_pos, SEEK_SET);
    if ((res = mov_write_moov_tag(pb, mov, s)) < 0)
        return res;
    moov_size = avio_tell(pb) - mov->reserved_header_pos;
    free_size = mov->reserved_moov_size - moov_size;
    if (free_size < 8) {
        av_log(s, AV_LOG_ERROR, "reserved_moov_size is too small, broken file\n");
        return AVERROR(EINVAL);
    }

    avio_wb32(pb, free_size);
    ffio_wfourcc(pb, "free");
    url_size = mov_write_url_to_free(s);
    flush_size = FFALIGN(moov_size + 8 + url_size, BLOCK_SIZE);
    skip_size = flush_size - (moov_size + 8 + url_size);
    #if (SSTAR_DEBUG)
    ffio_fill(pb, 0xB, skip_size);
    #else
    ffio_skip(pb, skip_size);
    #endif

    // align next mdat to cluster size for IO performance.
    avio_seek(pb, FFALIGN(current_pos, CLUSTER_SIZE), SEEK_SET);
    mov->mdat_size += FFALIGN(current_pos, CLUSTER_SIZE) - current_pos;
...
static int mov_write_single_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (mov->flags & FF_MOV_FLAG_FLUSH_MOOV_KEYFRAME &&
            ...
            pkt->pts - mov->flush_moov_pts > FLUSH_MOOV_INTERVAL &&
            mov->flush_moov_stream == pkt->stream_index) {
        ret = mov_auto_flush_moov(s);
        if (ret < 0)
            return ret;
        mov->flush_moov_pts = pkt->pts;
    }
...
static int mov_init(AVFormatContext *s)
{
    ...
    if (mov->reserved_moov_size) {
        mov->reserved_moov_size = FFALIGN(mov->reserved_moov_size, PAGE_SIZE);
    }

    if (mov->flags & FF_MOV_FLAG_FRAGMENT) {
        mov->flags &= ~FF_MOV_FLAG_FLUSH_MOOV_KEYFRAME;
    }
    ...
    if (mov->flags & FF_MOV_FLAG_FLUSH_MOOV_KEYFRAME) {
        mov->flush_moov_stream = -1;
        mov->flush_moov_pts = 0;
        for (i = 0; i < s->nb_streams; i++) {
            AVStream *st = s->streams[i];
            if (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
                mov->flush_moov_stream = i;
                break;
            }
        }
    }
...
static int mov_write_header(AVFormatContext *s)
{
    ...
    if (mov->reserved_moov_size) {
        ...
        if (mov->reserved_moov_size > 0) {
            avio_wb32(pb, mov->reserved_moov_size);
            ffio_wfourcc(pb, "free");
            #if (SSTAR_DEBUG)
            ffio_fill(pb, 0xF, BLOCK_SIZE - 8);
            #else
            ffio_skip(pb, BLOCK_SIZE - 8);
            #endif
            avio_seek(pb, mov->reserved_header_pos + mov->reserved_moov_size, SEEK_SET);
        }
    }
...
const AVCodecTag codec_mp4_tags[] = {
    ...
    { AV_CODEC_ID_MOV_TEXT    , MKTAG('t', 'x', '3', 'g') },
    { AV_CODEC_ID_BIN_DATA    , MKTAG('s', 's', 'm', 'd') },
