...
{
    MpegTSFilter *tss;
    int len, pid, cc, expected_cc, cc_ok, afc, is_start, is_discontinuity, is_random_access,
    ...
    is_random_access = has_adaptation &&
                       packet[4] != 0 && /* with length > 0 */
                       (packet[5] & 0x40); /* and random_access indicated */
    ...
    if (tss->type == MPEGTS_SECTION) {
        ...
        if (tss->type == MPEGTS_PES) {
            ...
            if (is_random_access) {
                // refs for mpegtsenc.c : set_af_flag(buf, 0x40);
                // need set flags after call pes_cb.
                PESContext *pc = tss->u.pes_filter.opaque;
                pc->flags |= AV_PKT_FLAG_KEY;
            }
        }
    }
