...
static const AVOption file_options[] = {
    { "truncate", "truncate existing files on write", offsetof(FileContext, trunc), AV_OPT_TYPE_BOOL, { .i64 = 0 }, 0, 1, AV_OPT_FLAG_ENCODING_PARAM },
    { "blocksize", "set I/O operation maximum block size", offsetof(FileContext, blocksize), AV_OPT_TYPE_INT, { .i64 = INT_MAX }, 1, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM },
    { "follow", "Follow a file as it is being written", offsetof(FileContext, follow), AV_OPT_TYPE_INT, { .i64 = 0 }, 0, 1, AV_OPT_FLAG_DECODING_PARAM },
    { "alignsize", "file size alignment", offsetof(FileContext, alignsize), AV_OPT_TYPE_INT, { .i64 = 0 }, 0, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM },
    { NULL }
};
...
static int file_write(URLContext *h, const unsigned char *buf, int size)
{
    ...
    if (c->alignsize && c->pos + size > c->filesize) {
        ret = fallocate(c->fd, FALLOC_FL_KEEP_SIZE, c->filesize, c->alignsize);
        if (ret == 0)
            c->filesize = c->filesize + c->alignsize;
    }

    c->syncsize += size;
    if (c->syncsize > 2 * 1024 * 1024) {
        fdatasync(c->fd);
        c->syncsize = 0;
    }

    ret = write(c->fd, buf, size);
    // printf("write pos[%llx] size[%x]\n", c->pos, size);
    posix_fadvise(c->fd, 0, c->pos + size, POSIX_FADV_DONTNEED);
...
static int64_t file_seek(URLContext *h, int64_t pos, int whence)
{
    ...
    // printf("seek cur [%llx] pos [%llx] whence [%d]\n", c->pos, pos, whence);

    if (whence == SEEK_SET && pos > c->filesize) {
        ret = fallocate(c->fd, FALLOC_FL_KEEP_SIZE, c->filesize, pos - c->filesize);
        if (ret == -1) {
            return AVERROR(errno);
        }
        c->filesize = pos;
...
static int file_close(URLContext *h)
{
    ...
    if (c->alignsize) {
        int alloc_size = c->alignsize - c->filesize % c->alignsize;
        if (alloc_size) {
            int ret = fallocate(c->fd,
                FALLOC_FL_KEEP_SIZE, c->filesize, alloc_size);
            if (ret == -1) {
                close(c->fd);
                return AVERROR(errno);
            }
    ...
    fdatasync(c->fd);
    posix_fadvise(c->fd, 0, c->filesize, POSIX_FADV_DONTNEED);
    return close(c->fd);
}
