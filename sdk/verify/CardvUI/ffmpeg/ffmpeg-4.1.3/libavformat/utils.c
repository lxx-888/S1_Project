...
FF_ENABLE_DEPRECATION_WARNINGS
        ...
        if (st->need_codec_info && st->need_parsing && !st->parser && !(s->flags & AVFMT_FLAG_NOPARSE)) {
...
static void estimate_timings_from_pts(AVFormatContext *ic, int64_t old_offset)
{
    ...
    int64_t duration_max_read_size;
    int retry = 0;
    ...
    filesize = ic->pb ? avio_size(ic->pb) : 0;
    duration_max_read_size = FFMIN(filesize / 20, DURATION_MAX_READ_SIZE);
    do {
...
FF_ENABLE_DEPRECATION_WARNINGS
        ...
        if (st->need_codec_info && !st->parser && !(ic->flags & AVFMT_FLAG_NOPARSE) && st->request_probe <= 0) {
            st->parser = av_parser_init(st->codecpar->codec_id);
            if (st->parser) {
    ...
    for (;;) {
         ...
         * the container. */
        if (st->need_codec_info && !has_codec_parameters(st, NULL)) {
            // printf("try decode frame [%d]\n", st->index);
    ...
    for (i = 0; i < ic->nb_streams; i++) {
        ...
        if (st->need_codec_info && !has_codec_parameters(st, &errmsg)) {
            char buf[256];
