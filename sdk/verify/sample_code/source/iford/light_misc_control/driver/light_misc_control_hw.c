...
static void* ir_pwm_handle = NULL;
#endif
#endif
...
static int SSTAR_Light_Misc_Control_Hw_Set_State(SSTAR_Switch_State_e state, bool bInIrq,
                                                 SSTAR_Light_Device_State_t* pLight)
{
    u8 gpio_dir = 0;
#if IS_ENABLED(CONFIG_SSTAR_PWM)
    u64                pwm_duty = 0;
    ...
    if (state == E_SWITCH_STATE_ON)
    {
        gpio_dir = 1;
#if IS_ENABLED(CONFIG_SSTAR_PWM)
        pwm_duty = gDeviceResource.IrCtrl.PwmData.duty;
#endif
    }
    ...
    if (0 == gDeviceResource.IrCtrl.u8CtrlMode)
    {
        ...
        camdriver_gpio_direction_output(NULL, gDeviceResource.IrCtrl.u32GpioId, gpio_dir);
    }
    else if (1 == gDeviceResource.IrCtrl.u8CtrlMode)
    {
         ...
         * value*/
        ir_pwm_sta.duty   = pwm_duty;
        ir_pwm_sta.period = gDeviceResource.IrCtrl.PwmData.period;

        /* The value of enable/polarity should not be changed */
        ir_pwm_sta.enabled  = 1;
        ir_pwm_sta.polarity = 0;

        if (NULL == ir_pwm_handle)
        {
            printk(KERN_ERR "ir pwm handle is null !!!\n");
        }
        else
        {
                                                 ...
                                                 bool bInIrq)
{
    ...
    if (openState == E_SWITCH_STATE_OFF)
    {
        if (eLightType == E_LIGHT_TYPE_IR)
        {
            SSTAR_Light_Misc_Control_Hw_Set_State(openState, bInIrq, pLight);
        }
        else if (eLightType == E_LIGHT_TYPE_LED)
        {
            Light_MISC_SPINLOCK(bInIrq, &pLight->switchStateSpinlock);
