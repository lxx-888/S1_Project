...
/************************************* Code ***********************************/
static void sstarCheckOta(char *upfile)
{
    if (access(upfile, F_OK) == 0 && strstr(upfile, ".zip")) {
        char* cmd = NULL;
        int status = -1;

        cmd = sfmt("busybox unzip -o %s -d %s", upfile, uploadDir);
        status = system(cmd);
        if (status < 0) {
            error("cannot decompress file.\n");
        } else {
            if (WIFEXITED(status) && WEXITSTATUS(status) == 0) {
                trace(2, "decompress ok, update file ready.");
                memset(cmd, 0, sizeof(cmd));
                sprintf(cmd, "/customer/wifi/run_ota.sh");
                if (access(cmd, F_OK) == 0) {
                    system("/config/bin/fw_setenv ota_upgrade 1");
                    trace(2, "run ota scripts file: %s", cmd);
                    status = system(cmd);
                    if (status < 0) {
                        error("cannot find ota scripts file.\n");
                    }
                }
                remove(upfile);
            } else
...
static bool uploadHandler(Webs *wp)
{
    ...
    if (fd >= 0) {
        ...
        close(fd);
        if (buffer[0] == '0') {
            error("SD card not exist\n");
            websError(wp, HTTP_CODE_NOT_FOUND | WEBS_NOLOG, "SD card not exist");
            return 0;
        }
    } else
        return 0;
...
PUBLIC void websUploadOpen(void)
{
    ...
    websDefineAction("upload", uploadHandler);
