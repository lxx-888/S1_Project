...
static unsigned long getTimeSinceMark(Webs *wp);
    ...
    if (wp->flags & WEBS_CLOSED) {
        ...
        if (wp->state < WEBS_READY) {
            if (wp->state > WEBS_BEGIN) {
                ...
                if ((end.tv_sec - start.tv_sec) > 1 && end.tv_usec > start.tv_usec) {
                    socketDeleteHandler(wp->sid);
                    trace(0, "Read error: 1S timeout!socketDeleteHandler...\n");
                    return;
                }
            } else {
                complete(wp, 0);
...
static void parseFirstLine(Webs *wp)
{
    ...
    if(strstr(wp->path, ME_GOAHEAD_UPLOAD_DIR))
    ...
    */
    for (count = 0; wp->rxbuf.servp[0] != '\r'; count++) {
         ...
         */
            ...
            RangeInput = (char*)malloc(strlen(value)+1);
            strcpy(RangeInput, value);
...
PUBLIC void websWriteHeaders(Webs *wp, ssize length, cchar *location)
{
    ...
    if (!(wp->flags & WEBS_HEADERS_CREATED)) {
        ...
        {
            websWriteHeader(wp, "Content-Range", "%s", wp->contentRange);
        }
        wp->txLen = length;
        if (wp->txLen < 0) {
...
static void checkTimeout(void *arg, int id)
{
    ...
    elapsed = getTimeSinceMark(wp);//Unit:ms
    if (websDebug) {
    ...
    if (elapsed >= WEBS_TIMEOUT) {
        ...
        //trace(0, "checkTimeout: %d, %x, %d", elapsed, wp->flags, wp->state);
        //wp->state = WEBS_COMPLETE;
        complete(wp, 0);
        websDone(wp);
        websFree(wp);
...
#else
{
    ...
    strncpy(ifr.ifr_name, "wlan0", IFNAMSIZ); // define host interface wlan0
    ifr.ifr_name[IFNAMSIZ - 1] = 0;
    if (ioctl(sockfd, SIOCGIFADDR, &ifr) < 0) {
        trace(0, "Interface wlan0: No such device. try p2p0...");
        strncpy(ifr.ifr_name, "p2p0", IFNAMSIZ); // define host interface p2p0
        ifr.ifr_name[IFNAMSIZ - 1] = 0;
        if (ioctl(sockfd, SIOCGIFADDR, &ifr) < 0) {
            trace(0, "Interface p2p0: No such device. try eth0...");
            strncpy(ifr.ifr_name, "eth0", IFNAMSIZ); // define host interface eth0
            ifr.ifr_name[IFNAMSIZ - 1] = 0;
            if (ioctl(sockfd, SIOCGIFADDR, &ifr) < 0) {
                trace(0, "Interface eth0: No such device.");
                closesocket(sockfd);
                return -1;
            }
        }
...
static unsigned long getTimeSinceMark(Webs *wp)
{
    return (GetTickCount() - wp->timestamp);
}
